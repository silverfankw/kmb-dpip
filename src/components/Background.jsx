export const Background = ({ uiMode = "night" }) => {
    ...
    const styles = {
        shell: `fixed inset-0 -z-10 overflow-hidden ${isLightMode ? "bg-[#fdfefe]" : "bg-[#0a1022]"}`,
        backdrop: isLightMode
        ...
        glowLeft: `absolute left-[10%] top-[8%] h-64 w-64 rounded-full blur-3xl ${isLightMode ? "bg-cyan-200/18" : "bg-cyan-300/14"}`,
        glowRight: `absolute bottom-[12%] right-[8%] h-72 w-72 rounded-full blur-3xl ${isLightMode ? "bg-violet-200/16" : "bg-violet-400/12"}`,
        overlay: `absolute inset-0 bg-linear-to-b from-transparent via-transparent ${isLightMode ? "to-white/45" : "to-slate-950/18"}`,
    }
